...
function sortBy(getId) {
	...
	if(sortId == 'varietySort') {
		...
		window.location.href = "polyhouseDisplay.php?sortByVariety=" + sortVariety;
		
	} else if(sortId == 'locationSort') {
		
		var sortLocation = document.getElementById('locationSort').value;
		window.location.href = "polyhouseDisplay.php?sortByLocation=" + sortLocation;
...
function callme(){
	...
	var table = tableToJson($('#polyhouseTable').get(0));
