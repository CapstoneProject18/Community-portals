...
function autoFill(x) {
	...
	if(x == 'noofbottles') {
		...
		var b = parseInt(document.getElementById("plantsperbottle").value);
		
		totalPlants = a*b;
	
	}
	
	if(x == 'plantsperbottle') {
		
		var b = parseInt(document.getElementById(x).value);
		var a = parseInt(document.getElementById("noofbottles").value);
	...
	document.getElementById('totalproduction').value = totalPlants;
...
function rowNumber(x) {
	...
	var noOfColumns = document.getElementById("greenhouseTable").rows[0].cells.length
	
	for(var i = 0; i < noOfColumns; i++) {
		
		var cellData = document.getElementById("greenhouseTable").rows[rowNo].cells[i].innerHTML;
