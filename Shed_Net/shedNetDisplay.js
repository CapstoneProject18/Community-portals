...
function sortBy(getId) {
	...
	if(sortId == 'varietySort') {
		...
		window.location.href = "shedNetDisplay.php?sortByVariety=" + sortVariety;
		
	} else if(sortId == 'locationSort') {
		
		var sortLocation = document.getElementById('locationSort').value;
		window.location.href = "shedNetDisplay.php?sortByLocation=" + sortLocation;
...
function callme(){
	...
	var table = tableToJson($('#shedNetTable').get(0));
