...
function sortBy(getId) {
	...
	if(sortId == 'varietySort') {
		...
		window.location.href = "greenhouseTRFDisplay.php?sortByVariety=" + sortVariety;
		
	} else if(sortId == 'locationSort') {
		
		var sortLocation = document.getElementById('locationSort').value;
		window.location.href = "greenhouseTRFDisplay.php?sortByLocation=" + sortLocation;
...
function callme(){
	...
	var table = tableToJson($('#greenhouseTRFTable').get(0));
